export default function IconBtn({ text, onclick, children, disabled, outline = false, customClasses, type, }) {
    return (
        <button
            disabled={disabled}
            onClick={onclick}
            className={`flex items-center justify-center outline-none ${outline ? "border border-richblack-50 bg-transparent" : "bg-richblack-50"
        ...
        >
            {
                children ? (
                    <>
                        <span className={`${outline && "text-richblack-50"}`}>{text}</span>
                        {children}
                    </>
                ) :
                    (text)
